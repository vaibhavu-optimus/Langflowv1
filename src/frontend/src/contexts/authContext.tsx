...
export function AuthProvider({ children }): React.ReactElement {
  ...
  function login(
    ...
    // Set cookies first
    cookies.set(LANGFLOW_ACCESS_TOKEN, newAccessToken, {
      path: "/",
      maxAge: 60 * 60 * 24 * 7, // 7 days
    });
    
    if (refreshToken) {
      cookies.set("refresh_token_lf", refreshToken, {
        path: "/",
        maxAge: 60 * 60 * 24 * 30, // 30 days
      });
    }
    
    // Let me look at the original function to see what it does
    // If the original login function doesn't update the store directly
    // then we don't need to do that here.
    
    // The getUser function will likely update the auth state based on the cookies
  getUser();
  }

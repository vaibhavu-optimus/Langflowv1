      ...
      <div className="card-shine-effect absolute inset-2 flex h-[calc(100%-16px)] min-w-[calc(100%-16px)] flex-col items-start gap-1 rounded-2xl p-4 text-white md:gap-3 lg:gap-4 lg:py-6">
        ...
        <div className="flex w-full items-center justify-between">
          <h3 className="line-clamp-3 text-lg font-bold lg:text-xl">
            {flow.name=="Vector Store RAG"?"CHATBOTS": flow.name=="Basic Prompting"?"Context Generation": flow.name=="Simple Agent"?"Agentic Workflow":flow.name}
          </h3>
        ...
        <p className="line-clamp-3 w-full overflow-hidden text-sm font-medium opacity-90">
          {flow.description=="Load your data for chat context with Retrieval Augmented Generation."?"Use Retrieval Augmented Generation to load your data for the conversation context.":flow.description}
        </p>
      </div>
    </div>
  ) : (
